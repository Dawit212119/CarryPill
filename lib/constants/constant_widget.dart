...
Widget loadingPillriveR(
  ...
  // Prefer Rive when asset exists; fallback avoids crash on incomplete clones.
  try {
    if (center) {
      return Center(
        child: SizedBox(
    ...
    final fallback = SizedBox(
      width: size.w,
      height: size.w,
      child: const CircularProgressIndicator(color: kcPrimary, strokeWidth: 3),
    );
    return center ? Center(child: fallback) : fallback;
  }
}
